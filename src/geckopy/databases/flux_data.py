...
class FluxData:
    ...
    tuning_rmse_weight
        Optional per-condition weights for the evolutionary-kcat-tuning
        RMSE term. ``None`` if the source file did not have the
        ``tuningRMSEweight`` column.
    source
    ...
    tuning_rmse_weight: Optional[np.ndarray] = None
...
def load_flux_data(path: str | Path) -> FluxData:
    ...
    * ``tuningRMSEweight`` (float): weights for evolutionary kcat tuning.
    * ``source`` (str): free-text describing the data origin.

    Both columns are dropped from the matrix and surfaced on the
    returned ``FluxData`` as ``tuning_rmse_weight`` and ``source``.
    ...
    tuning_idx = _index_of(header, "tuningRMSEweight")
    ...
    return FluxData(
        ...
        tuning_rmse_weight=tuning_weights,
